...
public class LongestConsecutiveSequence {
   ...
   *
   * - A brute force algo would be to sort the numbers, and iterate through sorted array to check if (previous value == current value - 1)
   ...
   *   ^-- O(n) time complex but less efficient in space complexity
   */
  public int computeSolution(int[] nums) {
    ...
    for (int num: uniqueNumbers) {
      ...
      int currentLength = 1;
      while (uniqueNumbers.contains(num++ + 1)) ++currentLength;
